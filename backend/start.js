...
app.post('/shop/add', async (req, res) => {
    try{
        const {initialState: [{sale, img, nameCard, cost, id, disable, inButton}]} = req.body
        const post = await Post.create({initialState: [{sale, img, nameCard, cost, id, disable, inButton}]})
        res.json(post)
    } catch(e) {
